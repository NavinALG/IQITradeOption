...
namespace ChartAndGraph
{
    public class GraphChart : GraphChartBase, ICanvas
    {
        ...
        public  List<Vector4> mTransformed = new List<Vector4>();
       ...
       public static float Yvalue=0.0f,Xvalue=0.0f;
        public float xMin, xMax;
        protected override void Update()
        {
            foreach(Vector4 abc in mTransformed)
            {
                Yvalue = abc.y - increment;
                Xvalue = abc.x;
            }
         
            pin.transform.localPosition = new Vector2(pin.transform.localPosition.x, Yvalue);
            pin.transform.GetChild(0).GetComponent<Text>().text = Yvalue.ToString();
            //midlebtntxt.text = Yvalue.ToString();
            midlebtntxt.text = pin.transform.GetChild(0).GetComponent<Text>().text;
            base.Update();
            HandleMouseDrag();
        ...
        public override void GenerateRealtime()
        {
            ...
            //Debug.Log(max);
            Rect viewRect = new Rect(0f, 0f, widthRatio/ RealtimeData, heightRatio);
